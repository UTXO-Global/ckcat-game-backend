...
import { TransactionsCrawlQueueName } from '../queue/crawl-transactions-queue.service'
...
import { parseCKBAddress } from '../../utils/helpers'
...
export class TransactionsCrawlService {
    ...
    getCells = async () => {
        ...
        const parse = parseCKBAddress(this.config.ckbAddress)
        ...
        try {
            let res = await axios.post(url, {
                ...
                params: [
                    {
                        script: {
                            code_hash: parse?.codeHash,
                            hash_type: parse?.hashType,
                            args: parse?.args,
                        },
                        script_type: 'lock',
                    },
    ...
    transactionsCrawl = async () => {
        try {
            ...
            for (const transaction of transactions) {
                const transactionDetail = await this.getTransaction(
                    transaction.out_point.tx_hash
                )
                ...
                if (
                    !order ||
                    transactionDetail.tx_status.status === order.status
                )
                    continue
                const capacities = transactionDetail.transaction.outputs.map(
                    (output) => {
                ...
                await startTransaction(async (manager) => {
                    ...
                    if (packageModel.type === PackageTypes.BuyGems) {
                        if (
                            transactionDetail.tx_status.status ===
                            TransactionStatusTypes.Committed
                        ) {
